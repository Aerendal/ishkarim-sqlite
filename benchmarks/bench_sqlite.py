#!/usr/bin/env python3
"""
bench_sqlite.py — benchmarki wydajnościowe dla ishkarim_sqlite.

Mierzy:
  1. Czas importu modułu
  2. Czas load_knowledge_index()
  3. Przepustowość extract_python_blocks()
  4. Testy specyficzne dla obszaru sqlite

Uruchomienie:
    python projects/ishkarim-sqlite/benchmarks/bench_sqlite.py
    python projects/ishkarim-sqlite/benchmarks/bench_sqlite.py --quick
"""
...
def bench_load_index(root: str, n: int = 3) -> tuple[float, int]:
    ...
    for _ in range(n):
        t0 = time.perf_counter()
        docs = ishkarim_sqlite.load_knowledge_index(root=root)
        times.append(time.perf_counter() - t0)
        count = len(docs)
    times.sort()
...
def bench_extract_blocks(root: str, n_files: int = 20) -> tuple[float, int]:
    ...
    from ishkarim_sqlite.utils import read_work_md, extract_python_blocks
    import ishkarim_sqlite
    docs = ishkarim_sqlite.load_knowledge_index(root=root)
    sample = docs[:n_files]
...
def run_area_benchmarks(root: str, quick: bool) -> list[tuple[str, str, str]]:
    """SQLite-specific: test WAL file detection and FTS5 presence."""
    results = []
    db_path = Path(root) / "tools" / "search.db"
    if not db_path.exists():
        return [("SQLite FTS5", "N/A", "search.db not found")]
    ...
    results.append(("SQLite table scan", fmt_ms(t), f"{count} rows, {len(tables)} tables"))
    ...
    t = time.perf_counter() - t0
    results.append(("PRAGMA integrity_check", fmt_ms(t), ok))

    return results


def main():
    parser = argparse.ArgumentParser(description="Benchmarki dla ishkarim-sqlite")
    ...
    print(f"\n============================================================")
    print(f"  Benchmark: ishkarim-sqlite")
    print(f"  Root: {root}")
